...
if __name__ == "__main__":
    ...
    # load trained model
    ckpt = 'ckpt/epoch=5-step=1362.ckpt'
    model = Transformer.load_from_checkpoint(ckpt)
    model.eval()

    while True:
        src = input('Enter text to be translated in to German : ').rstrip('\n')
        token_ids = torch.tensor(list(src_sp.encode_as_ids(src)))
        token_ids = torch.cat((torch.tensor([2]), token_ids, torch.tensor([3]))).unsqueeze(0)
